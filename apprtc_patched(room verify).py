...
#  };
  config = { 'iceServers': [{"urls":"turn:username@ip:3478?transport=udp", "credential":"password", "username":"username"}, {"urls":"turn:username@ip:3478?transport=tcp", "credential":"password", "username":"username"}] };
  if ice_transports:
...
class LeavePage(webapp2.RequestHandler):
  def post(self, room_id, client_id):
    #
    # [2016/11/21 12:53 KSH] AppRTC server logging.
    #
    logging.info("Logging room leave...")
    url = "http://logging-server/notifyRoomLeave"
    param = {"room_id": room_id}
    param_data = urllib.urlencode(param)
    req = urllib2.Request(url, param_data)
    data = urllib2.urlopen(req).read()
    logging.info("Logging result: " + data)
...
class JoinPage(webapp2.RequestHandler):
  ...
  def post(self, room_id):
    #
    # [2016/11/17 19:43 KSH] AppRTC server authentication.
    #  Check if the join requested room_id is from the api server. 
    # verifyRequest response result:
    #  "valid": valid room_id.
    #  "invalid(code)": invalid room_id.
    #
    logging.info("Verify room id...")
    url = "http://logging-server/verifyrequest"
    param = {"room_id": room_id}
    param_data = urllib.urlencode(param)
    req = urllib2.Request(url, param_data)
    data = urllib2.urlopen(req).read()
    logging.info("Verify result: " + data)
    if data is not None and data[0:5] == "valid":
      logging.info("Valid room_id!")
    else:
      logging.info("Invalid room_id!")
